...
<div class="mx-auto max-w-3xl space-y-6">
	<div class="flex items-center gap-4">
		<a href="/app/correspondences/{item.id}" class="rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100" aria-label={m.back()}>
			<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
			</svg>
		</a>
		<h1 class="text-2xl font-bold text-gray-900">{m.edit()} — {item.subject}</h1>
	</div>
	...
	<form method="post" use:enhance class="space-y-6 rounded-xl bg-white p-6 shadow-sm">
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			<div>
				...
				>
					<option value="incoming" selected={item.type === 'incoming'}>{m.incoming()}</option>
					<option value="outgoing" selected={item.type === 'outgoing'}>{m.outgoing()}</option>
				</select>
			</div>
			<div>
				...
				<select
					...
					class="mt-1 block w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500"
				>
					<option value="internal" selected={item.scope === 'internal'}>{m.internal()}</option>
					<option value="external" selected={item.scope === 'external'}>{m.external()}</option>
				</select>
			</div>
		</div>
			...
			<div>
				...
				>
					...
					{#each data.categories as cat}
						<option value={cat.id} selected={cat.id === item.categoryId}>{cat.name}</option>
					{/each}
				</select>
			</div>
			<div>
				...
				>
					...
					{#each data.correspondents as corr}
						<option value={corr.id} selected={corr.id === item.correspondentId}>{corr.name}</option>
					{/each}
				</select>
			</div>
		</div>
