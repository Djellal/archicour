...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-2xl font-bold text-gray-900">{m.correspondents()}</h1>
		<button
	...
	{#if showForm}
		<form method="post" action="?/create" use:enhance class="rounded-xl bg-white p-6 shadow-sm">
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
				<div>
					<label for="name" class="block text-sm font-medium text-gray-700">{m.correspondents()} *</label>
					<input
						id="name"
						type="text"
						name="name"
				...
				{#each data.correspondents as corr}
					<li class="flex items-center justify-between px-6 py-4">
						<div class="flex items-center gap-3">
							<p class="font-medium text-gray-900">{corr.name}</p>
							<span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium
								{corr.type === 'internal' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'}">
								{corr.type === 'internal' ? m.internal() : m.external()}
							</span>
						</div>
